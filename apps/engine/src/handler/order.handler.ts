...
import { sendResponse } from '../utils/send-ack';
...
export async function handleOpenTrade(
  ...
  try {
    ...
    if (!user) {
      ...
      await sendResponse(replyTo, requestId, 'TRADE_OPEN_FAILED', {
    ...
    if (
      ...
      await sendResponse(replyTo, requestId, 'TRADE_OPEN_FAILED', {
        reason: `Invalid slippage. Max allowed is ${MAX_SLIPPAGE_PERCENT}%`,
      });
      return;
    }
    ...
    if (slippedFraction > slippage / 100) {
      await sendResponse(replyTo, requestId, 'TRADE_SLIPPAGE_MAX_EXCEEDED', {
    ...
    await sendResponse(replyTo, requestId, 'TRADE_OPEN_ACKNOWLEDGEMENT', {
      status: 'success',
      tradeDetails: newTrade,
    });
  } catch (err) {
    console.error('Error in handleOpenTrade:', err);
    await sendResponse(replyTo, requestId, 'TRADE_OPEN_ERROR', { message: err });
  }
}
...
export async function handleCloseTrade(
  ...
  try {
    ...
    if (!user) {
      ...
      await sendResponse(replyTo, requestId, 'TRADE_CLOSE_FAILED', {
        reason: 'User not found',
      });
      return;
    }
    ...
    await sendResponse(replyTo, requestId, 'TRADE_CLOSE_ACKNOWLEDGEMENT', {
      status: 'success',
    });
  } catch (err) {
    console.error('Error in closing trade:', err);
    await sendResponse(replyTo, requestId, 'TRADE_CLOSE_ERROR', {
      message: err,
    });
  }
}
...
export async function handleFetchOpenOrders(
  ...
  try {
    ...
    if (!user) {
      ...
      await sendResponse(replyTo, requestId, 'TRADE_FETCH_FAILED', {
        reason: 'User not found',
      });
      return;
    }
    ...
    await sendResponse(replyTo, requestId, 'TRADE_FETCH_ACKNOWLEDGEMENT', {
      status: 'success',
      orders: orders,
    });
  } catch (err) {
    console.error('Error in handleFetchOpenOrders:', err);
    await sendResponse(replyTo, requestId, 'SOMETHING_WENT_WRONG', {
      message: err,
    });
  }
}
...
export async function handleFetchCandlesticks(
  ...
  try {
    ...
    await sendResponse(replyTo, requestId, 'CANDLESTICK_FETCH_ACKNOWLEDGEMENT', {
      status: 'success',
      candlesticks: candlesticks,
    });
  } catch (err) {
    await sendResponse(replyTo, requestId, 'CANDLESTICK_FETCH_ERROR', {
      message: err,
    });
  }
}